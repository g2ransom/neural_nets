...
class NeuralNet():
	...
	def sigmoid_gradient(self, z):
		if isinstance(z, np.ndarray), 'z must be an np.ndarray':
			...
			if  isinstance(iterations, int):
				for epoch in iterations:
					for i in range(self.m):
						...
						dels = []
						for l in reversed(range(self.theta_n - 1)):
							if l == (self.theta_n - 1):
								error_l = a[-1] - yy
								del_temp = error_l * sigmoid_gradient(np.dot(theta[l], a[l]))
								dels.append(del_temp)
							else:
								del_temp = np.multiply(np.dot(np.transpose(theta[l+1]), dels[-1]), sigmoid_gradient(np.dot(theta[l], a[l]))
								dels.append(del_temp)
